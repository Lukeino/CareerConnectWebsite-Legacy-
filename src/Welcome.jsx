...
function Welcome({ currentUser, searchTerm }) {
  const navigate = useNavigate();  const [recruiterStats, setRecruiterStats] = useState({
    annunciPubblicati: 0,
    ...
    const fetchRecruiterStats = async () => {
      ...
      try {
        ...
        if (data && data.data) {
          ...
          // Conta gli annunci chiusi (nascosti)
          const annunciChiusi = annunciDelRecruiter.filter(annuncio => {
            const stato = annuncio.stato || 
                         annuncio.attributes?.stato || 
                         'pubblicato'; // default se non specificato
            return stato === 'nascosto';
          }).length;

          setRecruiterStats({
            annunciPubblicati: annunciPubblicati,
            candidatureOttenute: 0, // Per ora 0 come richiesto
            annunciChiusi: annunciChiusi
          });

          console.log(`📊 Statistiche recruiter aggiornate: ${annunciPubblicati} annunci pubblicati, ${annunciChiusi} annunci chiusi`);
        }
      } catch (error) {
        console.error('Errore nel caricamento delle statistiche:', error);        // Mantieni i valori di default in caso di errore
        setRecruiterStats({
          annunciPubblicati: 0,
          candidatureOttenute: 0,
          annunciChiusi: 0
    ...
    <div className="welcome-container">      {/* Container trasparente superiore */}
      <div className="welcome-hero-container">
        <div className="hero-content">          <h1 className="hero-title">
            {currentUser ? (
              <>{getTimeBasedGreeting()}, {currentUser.username}!</>
            ) : (
          ...
          {currentUser?.roleType === 'recruiter' ? (
            <div className="recruiter-stats">
              ...
              <div className="stat-item">
                ...
                <div className="stat-number">{recruiterStats.annunciChiusi}</div>
                <div className="stat-label">Annunci chiusi</div>
              </div>
            </div>
          ) : (
            <button 
      ...
      <div className="welcome-main-container">
        ...
        <div className="dashboard-cards-container">          {/* Prima scheda - Lista Offerte */}          
          ...
          </div>          {/* Terza scheda - Gestisci Candidature (per recruiter) o Contatta i Recruiter (per altri) */}
          <div className="dashboard-card with-background" onClick={() => {
            if (currentUser?.roleType === 'recruiter') {
              navigate('/gestisci-candidature');
            } else {
              // Per altri utenti, non naviga da nessuna parte per ora
            }
          }}>
            <div className="card-icon-image">
              <img src={currentUser?.roleType === 'recruiter' ? ManageApplicationsImage : ConsulenceImage} 
                   alt={currentUser?.roleType === 'recruiter' ? 'Gestisci Candidature' : 'Contatta i Recruiter'} 
                   className="card-icon-img" />
            </div>
            <h3 className="card-title">
              {currentUser?.roleType === 'recruiter' ? 'Gestisci Candidature' : 'Contatta i Recruiter'}
            </h3>
            <p className="card-description">
              {currentUser?.roleType === 'recruiter' 
                ? 'Controlla tutte le candidature che hai ricevuto.'
