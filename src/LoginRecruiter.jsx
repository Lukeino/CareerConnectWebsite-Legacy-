...
function LoginRecruiter({ onLoginSuccess }) {
  ...
  const handleLoginSubmit = async e => {
    ...
    try {
      ...
      console.log("Full role data:", roleData); // Lascia questo per la verifica

      // Verifica che esista un ruolo e che sia 'recruiter'
      if (!roleData.data?.[0] || roleData.data[0].roleType !== 'recruiter') { // Modificato: rimosso .attributes
        console.log("Role data exists:", !!roleData.data?.[0]);
        if (roleData.data?.[0]) {
            console.log("User role value from data[0]:", roleData.data[0].roleType); // Modificato
        }
